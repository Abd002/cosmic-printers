//! Groups CUPS queues with their Printer Applications for display.

use std::collections::HashMap;

use crate::host::{hosts_match, parse_ip};
use crate::{PrinterApplication, PrinterEntry};

/// A normalized UUID, host-and-port endpoint, and URI used to group one queue or application.
...
impl DeviceIdentity {
    /// Normalizes a UUID, lowercases an endpoint host, and normalizes the device or fallback URI.
    fn new(
        uuid: Option<&str>,
        endpoint: Option<(String, u16)>,
    ...
    /// Matches an exact UUID, normalized URI, or compatible endpoint.
    ...
    /// Returns the available UUID, exact host-and-port, and normalized URI keys.
    ...
    /// Returns a location key; remote hosts omit ports so one CUPS server stays grouped.
...
/// Marks every item whose current disjoint-set component contains a Printer Application.
...
/// One Printers-list entry containing at most one Printer Application and its configured queues.
#[derive(Debug, Clone)]
pub struct GroupedDestination {
...
impl GroupedDestination {
    /// Returns the Printer Application in this entry, if present.
    pub fn printer_application(&self) -> Option<&PrinterApplication> {
        self.application.as_ref()
    }

    /// Returns the configured queues in this entry.
    pub fn queues(&self) -> &[PrinterEntry] {
        &self.queues
    }

    /// Returns the first normalized printer UUID collected for this entry.
    pub fn uuid(&self) -> Option<&str> {
        self.identity.uuid()
    }

    /// Returns the first normalized hostname collected for this entry.
    pub fn hostname(&self) -> Option<&str> {
        self.identity.hostname()
    }

    /// Returns the port paired with the hostname returned by [`Self::hostname`].
    pub fn port(&self) -> Option<u16> {
        self.identity.port()
    }

    /// Returns the first normalized device or printer URI collected for this entry.
...
impl GroupedDestination {
    ...
    /// Keeps the first available UUID, endpoint, URI, and Printer Application, then appends queues.
    /// The server orders applications by `(service_name, id)`, so duplicate records keep the first.
...
/// Groups exact identities first, then unclaimed queues by compatible host location.
pub fn group_printers(
    printers: Vec<PrinterEntry>,
    printer_applications: Vec<PrinterApplication>,
) -> Vec<GroupedDestination> {
    let items = printers
        .into_iter()
    ...
    // Freeze Printer Application membership before remote-host merging so an unclaimed queue
    // cannot connect a second host-only group to an application-owned group.
    ...
    let mut devices = Vec::<GroupedDestination>::new();

    for (index, item) in items.into_iter().enumerate() {
        let root = sets.find(index);
        if let Some(&slot) = slot_of_root.get(&root) {
            devices[slot].absorb(GroupedDestination::new(item));
        } else {
            slot_of_root.insert(root, devices.len());
            devices.push(GroupedDestination::new(item));
        }
    }
...
fn group_sort_key(device: &GroupedDestination) -> (&str, &str) {
...
/// Tests equal UUIDs, normalized URIs, or compatible endpoints.
...
/// Uses only the application's hostname and port. A PAPPL `system-uuid` can be shared by several
/// applications on one host, so using it would merge applications listening on different ports.
...
mod tests {
    ...
    use crate::host::host_is_known_local;
    ...
    fn group_of(groups: &[GroupedDestination], application_id: &str) -> Vec<String> {
        groups
            .iter()
            .find(|group| {
                group
