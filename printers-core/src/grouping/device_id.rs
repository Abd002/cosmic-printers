...
/// Free-text keys allowed to contain a semicolon followed by a fragment without `KEY:`.
/// Restricting continuation to `DES`, `DESCRIPTION`, and `COMMENT` prevents fragments from
/// changing manufacturer, model, serial-number, or command-set values used by grouping.
const CONTINUABLE_KEYS: &[&str] = &["DES", "DESCRIPTION", "COMMENT"];

/// A parsed IEEE-1284 device ID that retains its original form for driver matching.
    ...
    pub fn parse(raw: &str) -> Self {
        ...
        for token in raw.split(';') {
            ...
            match token.split_once(':') {
                ...
                // Only free-text fields may absorb malformed keyless fragments.
                None => {
