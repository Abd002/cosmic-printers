...
use super::destinations::raw_destination;
use super::routing::PRINTER_URI_SUPPORTED;
use super::scheduler::{self, CUPS_JOBS_URI, local_printer_uri};
...
pub async fn get_jobs(printer: &PrinterEntry, filter: &str) -> BackendResult<Vec<JobInfo>> {
    ...
    tokio::task::spawn_blocking(move || {
        ...
        // An advertised destination normally has no queue until a job is spooled.
        if matches!(
            response.status(),
...
pub async fn move_job(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let response = scheduler::send(request, CUPS_JOBS_URI)?;
...
/// Returns the local scheduler URI because it owns every submitted job ID.
...
fn parse_jobs(attributes: Vec<IppAttribute>, fallback_printer_id: &str) -> Vec<JobInfo> {
    // cups-rs hides group boundaries; a repeated name marks the next order-independent job group.
...
pub async fn print_test_page(printer: PrinterEntry) -> BackendResult<i32> {
    tokio::task::spawn_blocking(move || {
        ...
        let job = cups_rs::create_job(&destination, "Test Page").cups_err()?;
