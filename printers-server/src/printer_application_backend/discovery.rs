...
//!
//! A generation is one round of asking every Printer Application what it can see.
//! It exists because devices come and go: a candidate found two minutes ago may be
//! unplugged, and configuring it would send a request naming a device that is no
//! longer there. Every selection therefore quotes the generation it was made in,
//! and a request quoting an older one is refused rather than acted on.
//!
//! Applications answer at wildly different speeds — a device scan rescans USB,
//! SNMP, and DNS-SD — so results are published per application as they arrive
//! instead of waiting for the slowest. One application timing out does not hide
//! the others' results; the round finishes as complete-with-errors and says which
//! application failed.
...
/// Whether a scan state means the attempt broke rather than produced an answer.
///
/// `Complete` reporting nothing means the application looked and found nothing.
/// `Unsupported` means it cannot look. Both are answers. The rest are the attempt
...
/// Gives each row the identifier it already had in this round.
///
/// Applications answer one at a time and rows are published as they do, so the
/// evidence behind a row grows while the round runs. A row keyed on the strongest
/// evidence available would be renamed by that: a printer identified by its DNS-SD
/// service becomes identified by its UUID the moment a second application reports
/// the same printer and happens to include one. To anyone watching, the row
/// disappears and a new one takes its place — and a selection made against the old
/// identifier no longer resolves.
///
/// A row therefore keeps the identifier it was first given for as long as any of
/// the candidates it was built from are still in it. Identifiers stay unique
/// because a candidate belongs to one row at a time.
...
pub(crate) struct AddPrinterDiscovery {
    ...
    ///
    /// They are never selectable: resolving a selection only consults the current
    /// generation's snapshots, and the reply marks them as cached.
...
impl AddPrinterDiscovery {
    ...
    /// Starts a new generation for the given applications.
    ///
    /// Results from the previous round stop being selectable immediately, before
    /// any application has answered, because they describe devices that may
    /// already be gone.
    pub(crate) fn start(&mut self, applications: Vec<(String, String)>) -> DiscoveryGeneration {
        ...
        // A new round starts from what the last one found, not from nothing. Each
        // application's devices are replaced the moment it answers, so this only
        // shows through where an application fails to answer at all — and there,
        // what it reported a moment ago is a better account of the printers on this
        // desk than an empty list. The rows are reported as `cached` until something
        // fresh arrives, and cached rows cannot be configured.
    ...
    ///
    /// Ignores a stale generation, so a task from an abandoned round cannot write
    ...
    ///
    /// The replacement is atomic: an application's previous candidates are gone
    /// the moment its new ones land, so a caller never sees a mix of two rounds.
    /// Returns whether anything meaningful changed, so exactly one event is
    /// emitted per real change.
    pub(crate) fn replace_snapshot(
        ...
        // A scan that broke says nothing about what this application can see. If it
        // reported devices a moment ago, those are still the best account of them,
        // and replacing them with nothing is what makes a printer that had a driver
        // suddenly have none. A scan that *completed* and reported nothing is
        // different: that is an answer, and it is taken as one.
    ...
    ///
    /// Only its own candidates go; every other application's results stay, since
    /// one application disappearing says nothing about the others.
    ...
    /// Builds the reply a client reads.
    ///
    /// While a round is running with nothing fresh yet, the previous round's rows
    /// are returned marked `cached` so the view is not empty. Cached rows cannot
    /// be configured.
    ...
    /// Finds a candidate a client selected.
    ///
    /// Only the current generation's rows are searched. The candidate must belong
    /// to the row named, so an identifier lifted from another row cannot be used
    /// to configure a different printer.
    ...
    /// candidate.
    ///
    /// Checked again at configuration time: discovery may have finished a while
    /// ago, and the application may have stopped reporting the device since.
...
mod tests {
    use super::*;
    use crate::printer_application_backend::devices::DeviceTransport;
    use crate::printer_application_backend::drivers::PaDriver;
    use crate::printer_application_backend::identity::PaDeviceEndpoint;
