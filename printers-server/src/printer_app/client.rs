...
//! All operations share its endpoint, status mapping, and timeout policy.

use cups_rs::{IppOperation, IppRequest, IppResponse, IppStatus, IppTag, IppValueTag};
use std::time::Duration;

use crate::error::BackendError;
use crate::ipp::{CupsResultExt, IppTimeouts, add_requesting_user, send_to_with_timeouts};

/// Allows longer than device lookup because an application may be starting or busy.
...
pub(crate) enum PaError {
    ...
    /// The application refused the request, including unauthorized remote administration.
...
impl PaError {
    ...
    fn from_backend(error: BackendError) -> Self {
        match error {
            ...
            // A lost create response is not retried because it may have succeeded.
...
impl PaRequest {
    ...
    pub(super) fn send(
        ...
        let response = send_to_with_timeouts(self.request, system_uri, cost.timeouts())
            .map_err(PaError::from_backend)?;
    ...
    pub(super) fn send_allowing_failure(
        ...
        send_to_with_timeouts(self.request, system_uri, cost.timeouts())
            .map_err(PaError::from_backend)
    }
}
